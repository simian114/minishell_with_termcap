...
#### :heavy_exclamation_mark::heavy_exclamation_mark::heavy_exclamation_mark: 현재 시그널 부분에 문제가 있는듯
- 시그널을 키보드 입력으로 건네주게되면 부모, 자식 프로세스 모두가 그 시그널을 받게 된다. 시그널 함수를 fork() == 0 부분에서 만들고 자식 프로세스가 실행되는 경우의 시그널 핸들링을 만드는게 맞는 방법인거같다.
- iwoo 님이 만든 시그널 함수를 참고하자.
